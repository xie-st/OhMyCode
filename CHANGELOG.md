...
- `web_search`: proxy support via `OHMYCODE_PROXY` / `HTTPS_PROXY` / `HTTP_PROXY` environment variables — useful when DuckDuckGo is blocked on the local network (e.g. behind the GFW)

### Fixed
- `web_search`: clearer error message on connection failure, with a hint to set `OHMYCODE_PROXY` when DuckDuckGo is unreachable
