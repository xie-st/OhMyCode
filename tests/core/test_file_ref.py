...
def test_expand_single_ref(tmp_path):
    ...
    expanded, warnings = expand_file_refs("look at @hello.py please", str(tmp_path))
    assert warnings == []
...
def test_expand_multiple_refs(tmp_path):
    ...
    expanded, warnings = expand_file_refs("compare @a.py and @b.py", str(tmp_path))
    assert warnings == []
...
def test_expand_missing_file_leaves_token(tmp_path):
    expanded, warnings = expand_file_refs("look at @nonexistent.py", str(tmp_path))
...
def test_expand_no_refs(tmp_path):
    text = "hello world"
    expanded, warnings = expand_file_refs(text, str(tmp_path))
...
def test_expand_truncation_by_size(tmp_path):
    ...
    expanded, warnings = expand_file_refs("@big.txt", str(tmp_path))
    assert warnings == []
...
def test_expand_truncation_by_lines(tmp_path):
    ...
    expanded, warnings = expand_file_refs("@long.txt", str(tmp_path))
    assert warnings == []
    assert "truncated" in expanded.lower()
...
def test_expand_subdirectory(tmp_path):
    ...
    expanded, warnings = expand_file_refs("@src/main.py", str(tmp_path))
    assert warnings == []
...
def test_expand_non_utf8_file(tmp_path):
    ...
    expanded, warnings = expand_file_refs("@bin.dat", str(tmp_path))
    assert warnings == []
...
def test_expand_line_numbers_correct(tmp_path):
    ...
    expanded, _ = expand_file_refs("@lines.txt", str(tmp_path))
...
def test_expand_preserves_surrounding_text(tmp_path):
    ...
    expanded, _ = expand_file_refs("before @f.py after", str(tmp_path))
